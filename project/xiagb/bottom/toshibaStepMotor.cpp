...
namespace xiagb
{
    namespace motorNamespace
    {
        ...
        {
            float toshibaStepMotorClass::curve_s_table[curve_s_num];
            bool toshibaStepMotorClass::curve_s_data_build_flag;

            toshibaStepMotorClass::toshibaStepMotorClass()
            {
            }
            ...
            void toshibaStepMotorClass::curve_s_init()
            {
            ...
            void toshibaStepMotorClass::init(GPIO_TypeDef *step_gpiox, uint16_t step_gpio_x, GPIO_TypeDef *dir_gpiox, uint16_t dir_gpio_x, GPIO_TypeDef *reset_sensor_gpiox, uint16_t reset_sensor_gpio_x, GPIO_PinState reset_sensor_io_lv, GPIO_PinState reset_dir_io_lv, TIM_HandleTypeDef *power_timer_handle, uint8_t power_timer_channel, float run_power, float hold_power)
            {
                ...
                this->power_timer_handle = power_timer_handle;
                this->power_timer_channel = power_timer_channel;
                this->run_power = run_power;
                this->hold_power = hold_power;

                stop();
            }

            void toshibaStepMotorClass::move(int32_t need_run_step, uint16_t max_speed, uint16_t acc_dec_interval)
            {
                if (need_run_step == 0)
                {
                    stop();
                ...
                __HAL_TIM_SET_COMPARE(power_timer_handle, power_timer_channel, __HAL_TIM_GET_AUTORELOAD(power_timer_handle) * run_power);
                if (need_run_step > 0)
                {
            ...
            void toshibaStepMotorClass::move_to_position(uint16_t position, uint16_t max_speed, uint16_t acc_dec_action_interval)
            {
                //会将毫米转换成步数
            ...
            void toshibaStepMotorClass::motor_run_time_check()
            {
            ...
            void toshibaStepMotorClass::loop()
            {
                if (state == motor_stop)
                    return;
            ...
            void toshibaStepMotorClass::reset(uint16_t max_speed, uint16_t acc_dec_action_interval)
            {
                reset_flag = true;
                move(a_circle_step * -1000, max_speed, acc_dec_action_interval);
            }

            void toshibaStepMotorClass::stop()
            {
                state = motor_stop;
                stop_flag = true;
                keep_turn_flag = false;
                __HAL_TIM_SET_COMPARE(power_timer_handle, power_timer_channel, __HAL_TIM_GET_AUTORELOAD(power_timer_handle) * hold_power);
            }

            void toshibaStepMotorClass::keep_turn(bool dir, uint16_t max_speed, uint16_t acc_dec_action_interval)
            {
                keep_turn_flag = true;
                if (dir)
